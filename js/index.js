  ...
  fetch(url)
    ...
    .then((data) => {
        ...
        data.find((book) => {
          if (book.asin === ev.target.closest(".card-body").id) {
            ...
            const cartItemRemove =
              document.querySelectorAll(".cartItem_remove");
      ...
      const removeCartItem = (ev) => {
        ...
        const cartItemIndex = cartArr.findIndex(
          (item) => item.asin === ev.target.closest(".cartItem").id
        );
        ...
        for (const btn of filteredCardBtns) {
          if (
            btn.closest(".card-body").id === ev.target.closest(".cartItem").id
          ) {
